    ...
    return (
      <div className="flex items-center justify-center relative w-full h-full p-2">
        ...
        <div className="absolute inset-0 flex items-center justify-center flex-col pointer-events-none">
          ...
          <span className="text-[11px] font-black text-slate-600 dark:text-slate-200 uppercase tracking-tighter">Total</span>
        </div>
      </div>
    );
  };
    ...
    <div className="flex flex-col gap-4 animate-fadeIn h-[calc(100vh-100px)] overflow-hidden">
        ...
        {[
          ...
          <button key={i} onClick={() => toggleFilter('status', kpi.filter)} className={`flex-1 bg-white dark:bg-slate-800 border p-3 flex items-center gap-3 transition-all text-left relative ${activeFilters.status === kpi.filter ? `border-${kpi.color}-600 ring-2 ring-${kpi.color}-100 dark:ring-${kpi.color}-900 shadow-inner` : 'border-slate-200 dark:border-slate-700 panel-shadow hover:border-slate-400 dark:hover:border-slate-600'}`}>
            ...
            <div className="flex-1">
              <p className="text-[11px] font-black text-slate-600 dark:text-slate-200 uppercase tracking-widest leading-none mb-1">{kpi.label}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-4 flex-1 min-h-0">
        ...
        <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 panel-shadow flex flex-col overflow-hidden">
          <div className="bg-slate-50 dark:bg-slate-900/50 px-3 py-2 border-b border-slate-200 dark:border-slate-700 shrink-0">
            <h3 className="text-[11px] font-black text-slate-800 dark:text-slate-100 uppercase tracking-widest flex items-center gap-2">
              <Target size={14} className="text-slate-800 dark:text-slate-100" /> Indicadores Performance SLA
            </h3>
          </div>
            ...
            {[
              ...
              <div key={i} className="flex-1 flex flex-col border border-slate-100 dark:border-slate-700 shadow-sm min-h-0 overflow-hidden">
                <div className={`bg-${s.color}-50/50 dark:bg-${s.color}-900/20 px-2 py-1 border-b border-${s.color}-100 dark:border-${s.color}-900/50`}>
                  <p className={`text-[11px] font-black text-${s.color}-700 dark:text-${s.color}-300 uppercase tracking-[0.2em] leading-none`}>{s.label}</p>
                </div>
                <div className={`flex-1 flex flex-col justify-center p-2 border-l-4 border-${s.color}-600 bg-white dark:bg-slate-900 min-h-0`}>
                  <div className="grid grid-cols-4 gap-1">
                    ...
                    <div className="text-center">
                      ...
                      <p className={`text-[11px] font-black font-mono-tech text-slate-900 dark:text-slate-100`}>{s.target}</p>
                    </div>
                    <div className="text-center">
                      <p className="text-[11px] font-black text-slate-400 dark:text-slate-500 uppercase leading-none mb-1">SLA</p>
                      <p className={`text-[11px] font-black font-mono-tech ${s.pct >= 95 ? 'text-emerald-600' : 'text-amber-600'}`}>{`${Math.round(s.pct)}%`}</p>
                    </div>
                  </div>
                </div>
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 h-[40%] shrink-0">
        <div className="lg:col-span-3 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 panel-shadow flex flex-col overflow-hidden">
          <div className="bg-slate-50 dark:bg-slate-900/50 px-3 py-2 border-b border-slate-200 dark:border-slate-700 shrink-0 flex items-center justify-between">
            <h3 className="text-[11px] font-black text-slate-800 dark:text-slate-100 uppercase tracking-widest flex items-center gap-2">
              <BarChart3 size={14} className="text-slate-600 dark:text-slate-300" /> Market Share por CIA
            </h3>
              ...
              <div key={i} onClick={() => toggleFilter('cia', item.label)} className="flex-1 flex flex-col justify-center cursor-pointer group px-2 rounded hover:bg-slate-50 dark:hover:bg-slate-900/20">
                <div className="flex justify-between items-end mb-1">
                  <div className="flex items-center gap-1.5 overflow-hidden">
                    ...
                    <span className="text-[11px] font-black uppercase truncate text-slate-600 dark:text-slate-300 group-hover:text-slate-900 dark:group-hover:text-white">{item.label}</span>
                  </div>
                  <div className="flex items-baseline gap-2 shrink-0">
                    <span className="text-[12px] font-black font-mono-tech text-slate-900 dark:text-slate-100">{item.count}</span>
                    <span className="text-[11px] font-black text-indigo-500 dark:text-indigo-300 font-mono-tech">{item.pct.toFixed(1)}%</span>
                  </div>
                </div>
