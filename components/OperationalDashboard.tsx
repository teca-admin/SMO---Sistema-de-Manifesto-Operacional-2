...
export const OperationalDashboard: React.FC<OperationalDashboardProps> = ({ manifestos, onAction }) => {
  ...
  const getStatusStyle = (status: string) => {
    switch(status) {
      case 'Manifesto Iniciado': return 'bg-amber-100 text-amber-700 border-amber-200 dark:bg-amber-900/20 dark:text-amber-400 dark:border-amber-800/50';
      case 'Manifesto Recebido': return 'bg-blue-100 text-blue-700 border-blue-200 dark:bg-blue-900/20 dark:text-blue-400 dark:border-blue-800/50';
      case 'Manifesto Finalizado': return 'bg-emerald-100 text-emerald-700 border-emerald-200 dark:bg-emerald-900/20 dark:text-emerald-400 dark:border-emerald-800/50';
      case 'Manifesto Entregue': return 'bg-emerald-600 text-white border-emerald-700 dark:bg-emerald-700';
              ...
              <div className="absolute top-full left-0 right-0 mt-2 bg-white dark:bg-slate-800 border-2 border-slate-900 dark:border-slate-700 shadow-2xl z-[100] animate-fadeIn">
                {suggestions.map(s => (
                  ...
                  >
                    ...
                    <div className="p-2 rounded-full bg-slate-50 dark:bg-slate-900 group-hover:bg-indigo-100 dark:group-hover:bg-indigo-900 transition-colors">
  ...
  return (
    <div className="flex flex-col gap-8 animate-fadeIn">
      <div className="bg-[#0f172a] dark:bg-[#020617] border-2 border-slate-800 dark:border-slate-900 p-6 flex flex-col md:flex-row items-center justify-between shadow-xl">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-8 items-start">
        <div className="xl:col-span-4 space-y-4">
          <div className="flex items-center justify-between px-2">
            <h3 className="text-[11px] font-black text-slate-900 dark:text-slate-100 uppercase tracking-[0.2em] flex items-center gap-2">
          ...
          <div className="bg-white dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 panel-shadow overflow-hidden">
            {availablePool.length === 0 ? (
              <div className="p-12 text-center flex flex-col items-center">
                 <ShieldAlert size={32} className="text-slate-100 dark:text-slate-700 mb-2" />
                 <p className="text-[9px] font-black text-slate-300 dark:text-slate-600 uppercase italic">Nenhuma carga aguardando puxe no sistema</p>
              </div>
            ) : (
              <div className="divide-y divide-slate-100 dark:divide-slate-700">
                {availablePool.map(m => (
                  <div key={m.id} className="p-4 hover:bg-slate-50 dark:hover:bg-slate-900/30 transition-colors flex items-center justify-between group">
                    <div>
        ...
        <div className="xl:col-span-8 space-y-4">
          <div className="flex items-center justify-between px-2">
            ...
            <span className="text-[9px] font-black bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 px-2 py-0.5 rounded-full">Minhas Operações</span>
          </div>

          {myActiveLoads.length === 0 ? (
            <div className="bg-slate-50 dark:bg-slate-800/50 border-2 border-dashed border-slate-200 dark:border-slate-700 p-20 flex flex-col items-center justify-center text-center">
               <ShieldAlert size={48} className="text-slate-200 dark:text-slate-700 mb-4" />
               <h4 className="text-sm font-black text-slate-400 dark:text-slate-600 uppercase tracking-widest">Sem Cargas Atribuídas</h4>
               <p className="text-[9px] font-bold text-slate-400 dark:text-slate-600 uppercase mt-1">Puxe uma carga do Pool ao lado para iniciar o trabalho</p>
            </div>
          ) : (
                ...
                <div key={m.id} className={`bg-white dark:bg-slate-800 border-2 p-6 flex flex-col md:flex-row items-center justify-between gap-8 transition-all relative overflow-hidden ${m.status === 'Manifesto Iniciado' ? 'border-amber-400 shadow-xl' : 'border-slate-200 dark:border-slate-700 shadow-sm'}`}>
                   ...
                   <div className="flex flex-wrap items-center gap-8 flex-1">
                      <div className="min-w-[140px]">
                         ...
                         <p className="text-2xl font-black text-slate-900 dark:text-white font-mono-tech tracking-tighter leading-none">{m.id}</p>
                      </div>
                      <div className="h-10 w-px bg-slate-100 dark:bg-slate-700 hidden md:block"></div>
                      <div>
                         <p className="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1">Voo / CIA</p>
                         <p className="text-sm font-black text-slate-700 dark:text-slate-200 uppercase">{m.cia}</p>
                      </div>
                      <div className="h-10 w-px bg-slate-100 dark:bg-slate-700 hidden md:block"></div>
                      <div>
