...
import { Box, Play, CheckCircle2, UserCheck, Clock, Activity, Timer, AlertTriangle, ShieldAlert, Share2, Copy, Check, LayoutDashboard, Filter } from 'lucide-react';

interface KanbanBoardProps {
  manifestos: Manifesto[];
  isExternalView?: boolean;
  isAdmin?: boolean;
  urlCiaFilter?: string | null;
}

export const KanbanBoard: React.FC<KanbanBoardProps> = ({ manifestos, isExternalView = false, isAdmin = false, urlCiaFilter = null }) => {
  const [, setTick] = useState(0);
  const [copied, setCopied] = useState(false);
  const [copiedVinci, setCopiedVinci] = useState(false);
  ...
  // Aplica filtro de CIA se presente na URL ou prop
  const data = urlCiaFilter 
    ? manifestos.filter(m => m.cia.toUpperCase() === urlCiaFilter.toUpperCase())
    : manifestos;
  ...
  const columns = [
    {
      ...
      items: data.filter(m => m.status === 'Manifesto Recebido' && !m.usuarioResponsavel),
    ...
    },
    {
      ...
      items: data.filter(m => m.status === 'Manifesto Iniciado' || (m.status === 'Manifesto Recebido' && m.usuarioResponsavel)),
  ...
  const getSLAInfo = (m: Manifesto, colId: string) => {
    ...
    if (colId === 'recebido' && diffMin >= 60) return { status: 'warning', label: 'PONTO DE ATENÇÃO', diffMin };
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-[14px] font-black text-white uppercase tracking-[0.2em]">
              {isExternalView ? 'Visualização de Monitoramento' : 'Painel de Controle de Fluxo'}
            </h2>
            <div className="flex items-center gap-2">
               <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Live Sync Ativo</p>
               {urlCiaFilter && (
                 <div className="flex items-center gap-1.5 px-2 py-0.5 bg-indigo-600 text-white rounded-full text-[8px] font-black uppercase tracking-widest">
                   <Filter size={10} /> Filtro: {urlCiaFilter}
                 </div>
               )}
            </div>
          </div>
                  ...
                  return (
                    <div key={m.id} className={`bg-white dark:bg-slate-800 border p-3 shadow-sm hover:shadow-md transition-all group ${sla.status === 'critical' ? 'border-red-400' : sla.status === 'warning' ? 'border-amber-400' : 'border-slate-200 dark:border-slate-700'}`}>
                      <div className={`mb-2.5 px-2 py-1.5 rounded-sm flex items-center justify-between transition-colors ${barColor}`}>
                         <div className="flex items-center gap-1.5"><ShieldAlert size={11} className="text-white" /><span className="text-[9px] font-black uppercase tracking-widest text-white">{sla.label || 'Tempo em Status'}</span></div>
                      ...
                      <div className="grid grid-cols-2 gap-3 mb-3 px-1">
                        <div className="flex flex-col"><span className="text-[9px] font-bold text-slate-400 uppercase">Turno</span><span className="text-[10px] font-black text-slate-600 dark:text-slate-300 uppercase italic truncate">{m.turno}</span></div>
                        <div className="flex flex-col text-right"><span className="text-[9px] font-bold text-slate-400 uppercase">Início Log</span><span className="text-[10px] font-bold font-mono text-slate-500 dark:text-slate-400">{getTimeOnly(m.dataHoraPuxado)}</span></div>
                      </div>
                      <div className="pt-2.5 border-t border-slate-100 dark:border-slate-700 flex items-center justify-between">
                         <div className="flex items-center gap-2 max-w-[70%]">
                            <div className="w-5 h-5 bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-full flex items-center justify-center text-[10px] font-black text-slate-500 shrink-0">{m.usuarioResponsavel?.charAt(0) || '?'}</div>
                            <span className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase truncate">{m.usuarioResponsavel || 'Vago'}</span>
                         </div>
                         <div className={`flex items-center gap-1 text-[11px] font-black shrink-0 ${sla.status === 'critical' ? 'text-red-600' : 'text-indigo-500 dark:text-indigo-400'}`}><span className="font-mono-tech">{getTimeOnly(m.carimboDataHR)}</span></div>
                      </div>
                    </div>
                  );
                })
