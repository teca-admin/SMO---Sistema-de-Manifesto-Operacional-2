...
async function run() {
  console.log('\n=== INVESTIGAÇÃO: GAP ENTRE MANIFESTO PUXADO E MANIFESTO RECEBIDO ===\n');
  ...
  console.log(`Total carregado: ${all.length} manifestos com Puxado e Recebido\n`);
  ...
  for (const r of all) {
    ...
    if (gapH < 0) {
      casos.push({ ...r, dtPux, dtRec, gapH: Math.round(gapH * 10) / 10, tipo: 'INVERTIDO' });
    } else if (gapH >= 3) {
      casos.push({ ...r, dtPux, dtRec, gapH: Math.round(gapH * 10) / 10, tipo: 'GAP_GRANDE' });
    }
  }
  ...
  const buckets = { '3-6h': 0, '6-12h': 0, '12-24h': 0, '>24h': 0 };
  for (const c of grandes) {
    if (c.gapH < 6) buckets['3-6h']++;
    else if (c.gapH < 12) buckets['6-12h']++;
    else if (c.gapH < 24) buckets['12-24h']++;
    else buckets['>24h']++;
  }

  console.log('=== DISTRIBUIÇÃO DOS GAPS (Puxado → Recebido) ===\n');
  console.log(`  Gap ≥ 3h  : ${grandes.length} casos`);
  console.log(`    3h–6h   : ${buckets['3-6h']}`);
  console.log(`    6h–12h  : ${buckets['6-12h']}`);
  console.log(`    12h–24h : ${buckets['12-24h']}`);
  console.log(`    > 24h   : ${buckets['>24h']}`);
  console.log(`\n  Invertidos (Recebido < Puxado): ${invertidos.length} casos`);

  grandes.sort((a, b) => b.gapH - a.gapH);
  console.log('\n=== TOP 30 MAIORES GAPS ===\n');
  console.log('─'.repeat(100));
  for (const c of grandes.slice(0, 30)) {
    console.log(`\n  ID: ${c.ID_Manifesto}   CIA: ${c.CIA}   Turno: ${c.Turno}   Status: ${c.Status}`);
    console.log(`  Cadastrado por: ${c.Usuario_Sistema}`);
    console.log(`  Manifesto Puxado  : ${fmt(c.dtPux)}`);
    console.log(`  Manifesto Recebido: ${fmt(c.dtRec)}`);
    console.log(`  GAP: ${c.gapH}h`);
  }

  if (invertidos.length > 0) {
    console.log('\n\n=== CASOS INVERTIDOS (Recebido < Puxado) ===\n');
    for (const c of invertidos) {
      console.log(`  ID: ${c.ID_Manifesto}   CIA: ${c.CIA}   Status: ${c.Status}`);
      console.log(`  Puxado  : ${fmt(c.dtPux)} | Recebido: ${fmt(c.dtRec)} | GAP: ${c.gapH}h`);
    }
  }

  // Exporta CSV
  const linhas = ['ID_Manifesto,CIA,Status,Turno,Usuario,Puxado,Recebido,Gap_H,Tipo'];
  for (const c of casos) {
    linhas.push([c.ID_Manifesto, c.CIA, c.Status, c.Turno, c.Usuario_Sistema,
      fmt(c.dtPux), fmt(c.dtRec), c.gapH, c.tipo].join(','));
  }
  const fs = await import('fs');
  fs.writeFileSync('./scripts/puxado-recebido-gap.csv', linhas.join('\n'), 'utf8');
  console.log('\n\nCSV exportado: scripts/puxado-recebido-gap.csv\n');
  console.log(`Total geral: ${casos.length} casos anômalos\n`);
}
